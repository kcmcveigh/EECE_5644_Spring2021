...
% x_validate_cubic = [ones(1000,1) xValidate x_validate_squared x_validate_cubed];
% 
...
% x_train_cubic = [ones(100,1) xTrain x_train_squared x_train_cubed];
% 
...
load('q1_data_no_interactions/q1_x_train_cubic.txt');
load('q1_data_no_interactions/q1_x_validate_cubic.txt');
load('q1_data_no_interactions/q1_y_train.txt');
load('q1_data_no_interactions/q1_y_validate.txt');
%% fit weights ML
psuedo_inv = inv(q1_x_train_cubic'*q1_x_train_cubic)*q1_x_train_cubic';
weights_ml =psuedo_inv*q1_y_train;
y_hat_train_ml =weights_ml'*q1_x_train_cubic';
y_train_error = y_hat_train_ml' - q1_y_train;
sigma_error = mean(y_train_error.*y_train_error);
%% predict
...
for lambda_idx=1:length(lambda_values)
    prior_cov = (lambda_values(lambda_idx)/squared_error_ml)*eye(7);
...
prior_cov = lambda_values(min_lambda_idx)*eye(7);
map_inv = inv(q1_x_train_cubic'*q1_x_train_cubic +inv(prior_cov))*q1_x_train_cubic';
weights_map =map_inv*q1_y_train;
